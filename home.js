const { Client, Databases } = Appwrite;
  ...
  const newFindsBookContainer = document.getElementById("newfindsbookcontainer");
  
  const bestPicksBookContainer = document.getElementById("bestpicksbookcontainer");
  ...
  async function displayData() {
    ...
    books.forEach((book, index) => {
      if (index < 7){
        newFindsBookContainer.innerHTML += `
                <div class="col">
    ...
    for (let i=0; i < 5; i++){
      ...
      bestPicksBookContainer.innerHTML += ` <div class="col">
                <div class="card shadow-sm rounded-4 book-card" data-index="${randomIndex}" data-url="${books[randomIndex].url}">
                        ...
                        <div class="card-body">
                            <h5 class="card-title fw-bold">${books[randomIndex].title}</h5>
                        </div>
                    </div>
                </div>`
    }
...
document.getElementById("newfindsbookcontainer").addEventListener("click", function(event) {
...
document.getElementById("bestpicksbookcontainer").addEventListener("click", function(event) {
    let card = event.target.closest(".book-card");
    if (!card) return;
